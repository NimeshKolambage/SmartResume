...
function updateProfessionalTemplate(fullName, jobTitle, email, phone, location, website, summary, skillsInput, employmentInput, educationInput) {
    // Update header
    professionalElements.name.textContent = fullName.toUpperCase();
    professionalElements.jobTitle.textContent = jobTitle.toUpperCase();
...
function renderProfessionalSkills(skillsStr) {
    ...
    skillItem.className = 'prof-skill-category';
    skillItem.innerHTML = '<strong>Languages & Tools: </strong>' + skillsArray.join(', ');
    professionalElements.techSkills.appendChild(skillItem);
}

// ============================================
// RENDER PROFESSIONAL EDUCATION
// ============================================
...
function renderProfessionalEducation(educationStr) {
    ...
    entries.forEach(entry => {
        ...
        if (match) {
            ...
            html += `<div class="prof-item-subtitle">${match[2].trim()}${match[3] ? ` • ${match[3].trim()}` : ''}</div>`;
        } else {
            html += `<div class="prof-item-title">${entry}</div>`;
        }
        
        eduDiv.innerHTML = html;
        professionalElements.education.appendChild(eduDiv);
    });
}

// ============================================
// RENDER PROFESSIONAL EMPLOYMENT
// ============================================
...
function renderProfessionalEmployment(employmentStr) {
    ...
    entries.forEach(entry => {
        const firstLineMatch = entry.match(/^(.+?)\s*-\s*(.+?)(?:\s*\((.+?)\))?$/);
        
        if (firstLineMatch) {
            // This is a job title line
            if (currentJob) {
                professionalElements.employment.appendChild(currentJob);
            }
            
            currentJob = document.createElement('div');
            currentJob.className = 'prof-item';
            currentJob.innerHTML = `
                <div class="prof-item-title">${firstLineMatch[1].trim()}</div>
                <div class="prof-item-subtitle">${firstLineMatch[2].trim()}${firstLineMatch[3] ? ` • ${firstLineMatch[3].trim()}` : ''}</div>
            `;
        } else if (currentJob && entry) {
            // This is a description line
            const desc = document.createElement('div');
            desc.className = 'prof-item-bullet';
            desc.textContent = '• ' + entry;
            currentJob.appendChild(desc);
        }
    });
